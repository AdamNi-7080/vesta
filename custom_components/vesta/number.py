...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    comfort_temp = config.get(CONF_COMFORT_TEMP, DEFAULT_COMFORT_TEMP)
    entities: list[NumberEntity] = []

    for area in data.get("areas", {}).values():
        entities.append(VestaScheduleNumber(area, comfort_temp))
...
class VestaScheduleNumber(NumberEntity, RestoreEntity):
    ...
    def __init__(self, area: dict, default_target: float):
        ...
        self._attr_native_value = float(default_target)
